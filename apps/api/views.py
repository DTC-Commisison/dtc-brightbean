    ...
    "paths": {
        "/api/webhooks/asset-received": {
            "post": {
                ...
                "description": "Receives a rendered media asset, stores it in the workspace media library, and creates a draft Post with PlatformPost entries for each connected social account matching the requested platforms. HMAC-verified via X-Pencil-Signature header.",
                "tags": ["webhooks"],
                "requestBody": {
                    "required": True,
                    "content": {
...
def asset_received(request):
    ...
    1. Verify HMAC signature (X-Pencil-Signature: sha256=<hex>).
    2. Download asset from URL.
    3. Store in workspace MediaAsset (R2/S3 via Django storage).
    4. Create Post draft with PlatformPost entries for connected accounts.
    """
    # ── HMAC verification ────────────────────────────────────────────────
    ...
    filename = f"pencil-{payload[\"_id\"]}.{ext}"
    ...
    asset = MediaAsset(
        ...
        title=f"Pencil — {city or payload[\"_id\"]}",
    ...
    post = Post.objects.create(
        workspace=workspace,
        caption=caption or (f"FIFA 2026 Wine Corridor — {city.title()}" if city else ""),
